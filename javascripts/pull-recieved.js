...
(function pullPagePullImplementation($) {
  ...
  function gotPullDownData(event, data) {
   ...
   /* for (i=0; i<3; i+=1) {  // Generate some fake new content
      newContent = "<li><a href='#'>Pulldown-generated row " + (++pullDownGeneratedCount) + "</a></li>" + newContent;
      }*/
	// alert('loadhere');
	 var therandom = Math.floor((Math.random() * 100) + 1);
	 window.top.location.href = "load-request.html?url=requests-recieved.html&amp;ran="+therandom;
  ...
  function gotPullUpData(event, data) {
    var i,
        ...
        newContent = "";
    for (i=0; i<3; i+=1) { 
      newContent += "<li><a href='#'>Pullup-generated row " + (++pullUpGeneratedCount) + "</a></li>";
      }
